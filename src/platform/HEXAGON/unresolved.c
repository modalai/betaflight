...
// PA4 is the BMI270 chip-select pin on this target. IOGetByTag(PA4) hands
// out a pointer to this record so that a later IOLo() call can recognize
// the gyro CS and trigger the SPI mode-switch dummy read.
static ioRec_t hexagonPA4;
...
void IOLo(IO_t io)
{
    // bmi270EnableSPI() drives CS low/high to switch the BMI270 from its
    // power-on I2C mode into SPI mode. On HEXAGON the CS line is managed
    // by SLPI rather than us, so the IOLo/IOHi pair would otherwise be
    // no-ops; instead, when called on the gyro CS pin (PA4) we issue a
    // throwaway SPI transfer that produces the required CS edge.
    if (io == (IO_t)&hexagonPA4) {
        (void)hexagonSpiPerformBmi270DummyRead();
        delay(1);
    }
}
...
// No-op stub. CONFIG_IN_FILE is defined for the EEPROM-on-disk behavior
// (EEPROM_FILENAME); the headless CLI-provisioning path in main.c is not
// wired up on this platform yet, so returning NULL skips it.
