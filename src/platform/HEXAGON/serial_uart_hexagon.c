...
struct usartResource_s hexagon_uart[NUM_HEXAGON_UART];
...
void uartPinConfigure(const serialPinConfig_t *pSerialPinConfig)
{
	...
	// HEXAGON UARTs are logical DSP/SLPI endpoints, not pin-configured MCU
	// peripherals. Bind every compiled UART to its platform hardware entry so
	// the common UART layer recognizes it as available.
	for (const uartHardware_t *hardware = uartHardware; hardware < ARRAYEND(uartHardware); hardware++) {
		uartDevice_t *uartdev = uartDeviceFromIdentifier(hardware->identifier);
...
typedef enum {
	...
	UART_FUNCTION_MAVLINK
} hexagonUartFunction_e;

// Tag bytes used to multiplex virtual ports over the single SLPI link channel
// shared with the host bridge. Bytes also prepended on host->DSP direction.
#define HOST_TAG_MSP     0x5A
#define HOST_TAG_OSD     0xA5
#define HOST_TAG_REBOOT  0x42
#define HOST_TAG_MAVLINK 0x4D
...
// Receive buffer for MAVLink frames from host bridge (e.g. QGroundControl over UDP).
// Used when no rxCallback is registered (telemetry-only mode, which polls via
// hexagonSerialRead). When the serialrx provider registers a callback, incoming
// bytes are dispatched directly to that callback instead of being buffered.
#define MAVLINK_RX_BUFFER_LEN 2048
static uint8_t mavlink_rx_buffer[MAVLINK_RX_BUFFER_LEN];
static uint32_t _mav_rx_write = 0;
static uint32_t _mav_rx_read = 0;
static pthread_mutex_t _mav_rx_lock = PTHREAD_MUTEX_INITIALIZER;
static serialReceiveCallbackPtr mavlinkRxCallback = NULL;
static void *mavlinkRxCallbackData = NULL;

// Called by the SLPI LINK server when there is a new message for us from host side.
// Each message is a single host-bridge frame whose first byte is a tag identifying
// the destination virtual port. The payload (everything after the tag) is appended
// to that port's ring buffer.
...
int slpi_link_client_receive(const uint8_t *data, int data_len_in_bytes)
{
	...
	int payload_len = data_len_in_bytes - 1;

	if (tag == HOST_TAG_MAVLINK) {
		// Snapshot callback under the lock; invoke without holding it so the
		// callback (e.g. mavlinkDataReceive -> mavlink_parse_char dispatch)
		// can't deadlock against any caller that re-enters serial APIs.
		pthread_mutex_lock(&_mav_rx_lock);
		serialReceiveCallbackPtr cb = mavlinkRxCallback;
		void *cbData = mavlinkRxCallbackData;
		pthread_mutex_unlock(&_mav_rx_lock);

		if (cb != NULL) {
			for (int i = 0; i < payload_len; i++) {
				cb((uint16_t)payload[i], cbData);
			}
		} else {
			// Drop-oldest ring buffer: indices wrap modulo MAVLINK_RX_BUFFER_LEN,
			// one slot is sacrificed so empty (read == write) and full are
			// distinguishable. When a new payload doesn't fit, advance the
			// read pointer to make room rather than dropping the new frame —
			// the MAVLink parser will resync on the next start-of-frame byte.
			pthread_mutex_lock(&_mav_rx_lock);
			if (payload_len >= MAVLINK_RX_BUFFER_LEN) {
				// Payload larger than the entire ring: keep only the trailing
				// (BUFFER_LEN - 1) bytes.
				payload += payload_len - (MAVLINK_RX_BUFFER_LEN - 1);
				payload_len = MAVLINK_RX_BUFFER_LEN - 1;
				_mav_rx_read = 0;
				_mav_rx_write = 0;
			} else {
				uint32_t bytes_waiting = (_mav_rx_write - _mav_rx_read + MAVLINK_RX_BUFFER_LEN) % MAVLINK_RX_BUFFER_LEN;
				uint32_t bytes_free = MAVLINK_RX_BUFFER_LEN - 1 - bytes_waiting;
				if ((uint32_t)payload_len > bytes_free) {
					_mav_rx_read = (_mav_rx_read + ((uint32_t)payload_len - bytes_free)) % MAVLINK_RX_BUFFER_LEN;
				}
			}
			uint32_t first_chunk = MAVLINK_RX_BUFFER_LEN - _mav_rx_write;
			if ((uint32_t)payload_len <= first_chunk) {
				memcpy(&mavlink_rx_buffer[_mav_rx_write], payload, payload_len);
			} else {
				memcpy(&mavlink_rx_buffer[_mav_rx_write], payload, first_chunk);
				memcpy(&mavlink_rx_buffer[0], payload + first_chunk, payload_len - first_chunk);
			}
			_mav_rx_write = (_mav_rx_write + payload_len) % MAVLINK_RX_BUFFER_LEN;
			pthread_mutex_unlock(&_mav_rx_lock);
		}
	} else if (tag == HOST_TAG_MSP) {
		bool saved_data = true;
		pthread_mutex_lock(&_lock);
		if (_rx_write + payload_len < VIRTUAL_RX_BUFFER_LEN) {
			memcpy(&virtual_rx_buffer[_rx_write], payload, payload_len);
			_rx_write += payload_len;
		} else {
			saved_data = false;
		}
		...
		if (!saved_data) {
			printf("ERROR: Dropped %d incoming MSP bytes", payload_len);
		}
	} else {
		// Back-compat: an older host bridge sends MSP bytes with no tag.
		// Treat untagged traffic as MSP.
		bool saved_data = true;
		pthread_mutex_lock(&_lock);
...
uint32_t hexagonSerialTotalRxWaiting(const serialPort_t *instance) {
	...
	} else if (uartFunction == UART_FUNCTION_MAVLINK) {
		pthread_mutex_lock(&_mav_rx_lock);
		bytes_waiting = (_mav_rx_write - _mav_rx_read + MAVLINK_RX_BUFFER_LEN) % MAVLINK_RX_BUFFER_LEN;
		pthread_mutex_unlock(&_mav_rx_lock);
...
uint8_t hexagonSerialRead(serialPort_t *instance) {
	...
	if (uartFunction == UART_FUNCTION_MSP) {
		...
		pthread_mutex_lock(&_mav_rx_lock);
		if (_mav_rx_write != _mav_rx_read) {
			byte_data = mavlink_rx_buffer[_mav_rx_read];
			_mav_rx_read = (_mav_rx_read + 1) % MAVLINK_RX_BUFFER_LEN;
		}
		pthread_mutex_unlock(&_mav_rx_lock);
	} else if (uartFunction == UART_FUNCTION_HW) {
		int hw_index = getUARTHWindex(port_number);
...
static int mavlinkTxBufferIndex = 0;
static uint8_t mavlinkTxBuffer[4096];
static bool mavlinkFlush = false;
static pthread_mutex_t mavlink_mutex = PTHREAD_MUTEX_INITIALIZER;
...
void *tx_thread(void *arg)
{
	...
	while (true) {
		...
		if (_reset) {
			...
			uint8_t resetCmd[1] = { HOST_TAG_REBOOT };
		...
		// Handle MAVLink processing for ground station connection
		if (mavlinkFlush) {
			pthread_mutex_lock(&mavlink_mutex);
			(void) sl_client_send_data(mavlinkTxBuffer, mavlinkTxBufferIndex);
			mavlinkFlush = false;
			mavlinkTxBufferIndex = 1;
...
void hexagonWriteBuf(serialPort_t *instance, const void *data, int count) {
	...
	if (uartFunction == UART_FUNCTION_MSP) {
		...
		if (mavlinkTxBufferIndex + count > 4096) {
			printf("ERROR: MAVLink tx buffer overflow");
		} else {
			pthread_mutex_lock(&mavlink_mutex);
			memcpy(&mavlinkTxBuffer[mavlinkTxBufferIndex], data, count);
			mavlinkTxBufferIndex += count;
			mavlinkFlush = true;
			pthread_mutex_unlock(&mavlink_mutex);
		}
...
void hexagonSerialWrite(serialPort_t *instance, uint8_t ch) {
	...
	if (uartFunction == UART_FUNCTION_BLACKBOX) {
		...
		// MAVLink telemetry writes one byte at a time; the tx_thread (1ms cadence)
		// coalesces accumulated bytes into a single tagged sl_client_send_data call.
		pthread_mutex_lock(&mavlink_mutex);
		if (mavlinkTxBufferIndex < 4096) {
			mavlinkTxBuffer[mavlinkTxBufferIndex++] = ch;
			mavlinkFlush = true;
		} else {
			printf("ERROR: MAVLink tx buffer overflow (serialWrite)");
		}
		pthread_mutex_unlock(&mavlink_mutex);
	}
}
...
uartPort_t *serialUART(uartDevice_t *uartdev, uint32_t baudRate, portMode_e mode, portOptions_e options)
{
	...
	if (uartFunction == UART_FUNCTION_HW) {
		...
		printf("Configuring MAVLink virtual port");
	} else if (uartFunction == UART_FUNCTION_OSD) {
...
static void startTxThreadOnce(void)
{
	...
	int tx_priority = 128;
	printf("Setting host-bridge Tx pthread priority to %d", tx_priority);

	struct sched_param param = { .sched_priority = tx_priority };
	pthread_attr_t attr;
...
void uartReconfigure(uartPort_t *uartPort)
{
	...
	if (uartFunction == UART_FUNCTION_HW) {
		...
		mspTxBuffer[0] = HOST_TAG_MSP;
		mspTxBufferIndex = 1;
	} else if (uartFunction == UART_FUNCTION_MAVLINK) {
		printf("Reconfiguring MAVLink virtual port (rxCallback %s)",
			uartPort->port.rxCallback ? "set" : "unset");
		mavlinkTxBuffer[0] = HOST_TAG_MAVLINK;
		mavlinkTxBufferIndex = 1;

		// Capture rx callback so slpi_link_client_receive can dispatch bytes
		// directly when serialrx_provider = MAVLINK opens the port for RX.
		pthread_mutex_lock(&_mav_rx_lock);
		mavlinkRxCallback = uartPort->port.rxCallback;
		mavlinkRxCallbackData = uartPort->port.rxCallbackData;
		pthread_mutex_unlock(&_mav_rx_lock);
