...
#define TASK_GYROPID_DESIRED_PERIOD     312     // 312us = ~3.2kHz, matches BMI270 native ODR
...
#define USE_UART4
#define USE_UART5
#define USE_UART6      // Virtual UART for MAVLink (host bridge)
#define USE_UART7
#undef USE_UART8

#define USE_SERIALRX
#define USE_SERIALRX_CRSF
// common_pre.h gates USE_SERIALRX_MAVLINK on TARGET_FLASH_SIZE, but target.h is
// included AFTER common_pre.h, so that gate always misses on this build. Define
// it explicitly here so rx/rx.c compiles the SERIALRX_MAVLINK dispatch case.
...
// VOXL internal DPS368 is wired with SDO high → secondary I2C address.
...
// Concrete definition of the opaque spiResource_s declared in bus_spi_types.h.
// Generic code references SPI peripherals through spiResource_t pointers and never
// peeks inside; HEXAGON platform code accesses ->fd directly.
struct spiResource_s
{
    int fd;
};

extern struct spiResource_s hexagon_spi_bus;

#define SPI1 ((struct spiResource_s *) &hexagon_spi_bus)

#define __set_BASEPRI(VALUE) ((void) VALUE)
#define __get_BASEPRI(VALUE) (0)
#define __set_BASEPRI_MAX(VALUE) ((void) VALUE)

// Concrete definition of the opaque usartResource_s declared in serial_uart_types.h.
// Generic code references UART peripherals through usartResource_t pointers;
// HEXAGON platform code accesses ->fd / ->port_number / ->speed directly.
struct usartResource_s
{
    int fd;
	uint8_t port_number;
	uint32_t speed;
...
extern struct usartResource_s hexagon_uart[NUM_HEXAGON_UART];

#define USART1 ((struct usartResource_s *) &hexagon_uart[0])
#define USART2 ((struct usartResource_s *) &hexagon_uart[1])
#define USART3 ((struct usartResource_s *) &hexagon_uart[2])
#define USART4 ((struct usartResource_s *) NULL)
#define USART5 ((struct usartResource_s *) NULL)
#define USART6 ((struct usartResource_s *) &hexagon_uart[6])
#define USART7 ((struct usartResource_s *) NULL)
#define USART8 ((struct usartResource_s *) NULL)

#define UART4 ((struct usartResource_s *) &hexagon_uart[3])
#define UART5 ((struct usartResource_s *) &hexagon_uart[4])
#define UART6 ((struct usartResource_s *) &hexagon_uart[6])
#define UART7 ((struct usartResource_s *) &hexagon_uart[5])
#define UART8 ((struct usartResource_s *) NULL)
...
#define MAVLINK_UART SERIAL_PORT_USART6 // Virtual UART, tag 0x4D via host bridge
